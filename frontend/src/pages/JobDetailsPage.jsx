...
import { ArrowLeft, MapPin, Clock, Briefcase, IndianRupee, Users, Calendar } from "lucide-react";
...
export default function JobDetailsPage() {
  ...
  const renderStageContent = () => {
    switch (activeStage) {
      case "sourcing":
        return <SourcingWorkflow jobId={job.id} />;
      case "recruitment":
        return (
          <EmptyStage
            title="Screening"
            description="Candidates in the screening phase will appear here. Move candidates from sourcing to begin screening."
        ...
        <div className="h-16 flex items-center gap-4 px-8 border-b border-slate-200 bg-white flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-0.5 text-xs text-slate-400">
              ...
              <span className="flex items-center gap-1">
                <Calendar className="h-3 w-3" /> Posted {job.posted}
              </span>
            </div>
          </div>
...
function EmptyStage({ title, description, count }) {
  return (
    <div data-testid={`stage-${title.toLowerCase()}`} className="flex flex-col items-center justify-center py-20">
      ...
      <h3 className="text-lg font-semibold text-slate-700 mb-1" style={{ fontFamily: "Manrope, sans-serif" }}>
        {title}
      </h3>
